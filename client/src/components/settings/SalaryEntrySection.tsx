...
export function SalaryEntrySection() {
  ...
  const handleSubmit = async () => {
    if (!notes.trim()) { addToast('Please enter notes (e.g. "Salary March 25")', 'error'); return; }
    if (!date)         { addToast('Please select a date', 'error'); return; }
    if (settings.activeCashBookType === 'Both') {
      addToast('Switch to Aided or Un-Aided to add salary entries', 'error'); return;
    }
    ...
    if (entries.length === 0) { addToast('No amounts entered — nothing to save', 'error'); return; }

    setSubmitting(true);
    try {
      await Promise.all(entries.map(e => apiCreateEntry(e)));
      addToast(`${entries.length} salary ${entries.length === 1 ? 'entry' : 'entries'} saved`, 'success');
      reset();
    } catch (err) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white overflow-clip">
      ...
      {/* ── Shared fields ── */}
      <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[180px]">
          <label className="block text-xs font-medium text-slate-600 mb-1">Salary Month</label>
          <input
            type="text"
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="e.g. Mar 26"
            className="w-full h-8 rounded-md border border-slate-200 bg-white px-2.5 text-sm
              text-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-300
              placeholder:text-slate-300"
          />
        </div>
        <div>
          <DateInput label="Date" value={date} onChange={setDate} className="h-8 py-1 w-32" />
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-600 mb-1">Cheque / Mode</label>
          <input
            type="text"
            value={cheque}
            onChange={e => setCheque(e.target.value)}
            className="h-8 w-24 rounded-md border border-slate-200 bg-white px-2.5 text-sm
              text-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-300"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-slate-200">
        ...
        <div className="flex flex-col">
          <div className="bg-green-50 border-b border-green-100 px-3 py-1.5">
            <span className="text-xs font-semibold text-green-700 uppercase tracking-wider">Receipts</span>
          </div>
          <div className="divide-y divide-slate-100">
            {RH.map(h => (
              <div key={h} className="flex items-center justify-between gap-3 px-3 py-1.5">
                <span className="text-xs text-slate-700 min-w-0 flex-1 font-medium">{RECEIPT_LABELS[h]}</span>
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <input
                    ...
                    className="h-7 w-32 rounded border border-slate-200 bg-white px-2 text-sm text-right
          ...
          <div className="mt-auto bg-green-50/60 border-t border-green-100 px-3 py-1.5 flex justify-between items-center">
            ...
            <span className="text-xs font-semibold text-green-700 tabular-nums">{formatCurrency(totalR)}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <div className="bg-red-50 border-b border-red-100 px-3 py-1.5">
            <span className="text-xs font-semibold text-red-700 uppercase tracking-wider">Payments</span>
          </div>

          <div className="divide-y divide-slate-100">
            {/* Govt Salary Acct — manual */}
            <div className="flex items-center justify-between gap-3 px-3 py-1.5">
              <span className="text-xs text-slate-700 min-w-0 flex-1 font-medium">Salary Acct</span>
              <div className="flex items-center gap-1 shrink-0">
                <span className="text-[10px] text-slate-400">₹</span>
                <input
                  type="number"
            ...
            <div className="flex items-center justify-between gap-3 px-3 py-1.5 bg-slate-50/60">
              <div className="min-w-0 flex-1">
                <div className="text-xs text-slate-700 font-medium">Salary Receivable</div>
                <div className="text-[10px] text-slate-400 mt-0.5">Auto · LIC + I Tax + P Tax + GSLIC + FBF</div>
              </div>
          ...
          <div className="mt-auto bg-red-50/60 border-t border-red-100 px-3 py-1.5 flex justify-between items-center">
            ...
            <span className="text-xs font-semibold text-red-700 tabular-nums">{formatCurrency(totalP)}</span>
          </div>
        </div>
      ...
      <div className="border-t border-slate-200 bg-slate-50 px-4 py-2.5 flex items-center justify-between gap-3">
        <span className="text-xs text-slate-400 truncate">
          {notes.trim()
            ? <>
                <span className="font-medium text-slate-600">{toProperCase(expandMonth(notes.trim()))}</span>
                {' · '}{date}{' · '}{cheque || 'Neft'}
        ...
        <div className="flex gap-2 shrink-0">
          <button
            type="button" onClick={reset} disabled={submitting}
          ...
          <button
            ...
            className="h-8 rounded-md bg-blue-600 px-5 text-sm font-semibold text-white
