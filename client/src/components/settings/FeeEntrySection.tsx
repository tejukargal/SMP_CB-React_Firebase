...
function buildNotes(raw: string): string {
  ...
  if (m) return `College Fee Collection, Rpt No From: ${m[1]} To: ${m[2]}`;
  return raw.trim();
}
...
function FeeRow({ head, value, onChange }: {
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-3 py-1.5">
      <span className="text-xs text-slate-700 min-w-0 flex-1 truncate font-medium">{head}</span>
      <div className="flex items-center gap-1 shrink-0">
        ...
        <input
          ...
          className="h-6 w-24 rounded border border-slate-200 bg-white px-1.5 text-xs text-right
...
export function FeeEntrySection() {
  ...
  const handleSubmit = async () => {
    ...
    if (!notesRaw.trim()) { addToast('Please enter a receipt number or notes', 'error'); return; }
    if (settings.activeCashBookType === 'Both') {
      addToast('Switch to Aided or Un-Aided to add fee entries', 'error'); return;
    }
    ...
    for (const h of FEE_HEADS) {
      ...
      if (!isNaN(amt) && amt > 0) {
        entries.push({ date, chequeNo: canonicalCheque, amount: amt,
          headOfAccount: h, notes, type: 'Receipt', financialYear: fy, cashBookType: cbType });
      }
    }
    ...
    try {
      ...
      addToast(`${entries.length} fee ${entries.length === 1 ? 'entry' : 'entries'} saved`, 'success');
      reset();
    } catch (err) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white overflow-clip">
      ...
      <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-3 items-end">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-600 mb-1">Cheque No / Mode</label>
          <input
            type="text"
            value={cheque}
            onChange={e => setCheque(toProperCase(e.target.value))}
            placeholder="e.g. 123456 or Cash"
            className="h-8 w-36 rounded-md border border-slate-200 bg-white px-2.5 text-sm
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs font-medium text-slate-600 mb-1">
            Receipt No / Notes
            <span className="ml-1 text-slate-400 font-normal">(e.g. 5890-5895)</span>
          </label>
          <input
            type="text"
            value={notesRaw}
      ...
      <div>
        <div className="bg-green-50 border-b border-green-100 px-3 py-1.5 flex items-center justify-between">
          <span className="text-xs font-semibold text-green-700 uppercase tracking-wider">Fee Receipts</span>
        ...
        <div className="bg-green-50/60 border-t border-green-100 px-3 py-1.5 flex justify-between items-center">
          ...
          <span className="text-xs font-semibold text-green-700 tabular-nums">{formatCurrency(total)}</span>
        </div>
      </div>
      ...
      <div className="border-t border-slate-200 bg-slate-50 px-4 py-2.5 flex items-center justify-between gap-3">
        <span className="text-xs text-slate-400 truncate">
          {notesPreview
            ? <><span className="font-medium text-slate-600">{notesPreview}</span>{' · '}{date}{cheque && <>{' · '}{cheque}</>}</>
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <button
            ...
            className="h-8 rounded-md bg-green-600 px-5 text-sm font-semibold text-white
